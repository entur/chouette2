...
RUN apt-get update \
    && apt-get -y upgrade \
    && apt-get install -y nodejs libgeos-dev proj-bin libproj-dev sendmail dumb-init shared-mime-info sqlite3 postgresql-client wget \
    && apt-get clean
...
RUN apt-get update \
    && apt-get install -y build-essential libpq-dev libsqlite3-dev git unzip make less patch vim \
    && apt-get clean
